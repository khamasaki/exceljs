class PivotTable {
  ...
  validate() {
    ...
    pivotTable.values.forEach(value => {
      ...
      if (value.showDataAs) {
        assert(value.baseField, 'Must have a baseField when showDataAs is set');
        assert(allShowDataAs.includes(value.showDataAs), `Invalid pivot table showDataAs value: ${value.showDataAs}`);
      }
      if (value.baseField) {
        const baseField = this.pivotTable.pivotCache.cacheFields.find(cf => cf.name === value.baseField);
        assert(baseField, `Invalid pivot table baseField: ${value.baseField}`);
      }
    });
  ...
  get dataFields() {
    ...
    return this.pivotTable.values.map(value => ({
      ...
      baseField: value.baseField ? this.pivotFields.findIndex(item => item.name === value.baseField) : undefined,
    }));
  }

...
const PivotCacheDefinitionXform = require('./xform/pivot/pivot-cache-definition-xform');
const PivotCacheRecordsXform = require('./xform/pivot/pivot-cache-records-xform');
const PivotTableXform = require('./xform/pivot/pivot-table-xform');
...
class XLSX {
  ...
  async addWorkbookRels(zip, model) {
    ...
    model.pivotCaches.forEach(pivotCache => {
      pivotCache.rId = `rId${count++}`;
      relationships.push({
        Id: pivotCache.rId,
        Type: XLSX.RelType.PivotCacheDefinition,
        Target: `pivotCache/pivotCacheDefinition${pivotCache.id}.xml`,
      });
    });
    const xform = new RelationshipsXform();
  ...
  async addPivotCacheDefinitions(zip, model) {
    const pivotCacheDefinitionXform = new PivotCacheDefinitionXform();
    const pivotCacheRecordsXform = new PivotCacheRecordsXform();
    const relationshipsXform = new RelationshipsXform();

    model.pivotCaches.forEach(pivotCache => {
      pivotCacheDefinitionXform.prepare(pivotCache, {});
      const xml = pivotCacheDefinitionXform.toXml(pivotCache);
      zip.append(xml, {name: `xl/pivotCache/pivotCacheDefinition${pivotCache.id}.xml`});

      const recXml = pivotCacheRecordsXform.toXml(pivotCache);
      zip.append(recXml, {name: `xl/pivotCache/pivotCacheRecords${pivotCache.id}.xml`});

      const relXml = relationshipsXform.toXml(pivotCache.rels);
      zip.append(relXml, {name: `xl/pivotCache/_rels/pivotCacheDefinition${pivotCache.id}.xml.rels`});
    });
  }
  ...
  async addPivotTables(zip, model) {
    ...
    model.pivotTables.forEach(pivotTable => {
      pivotTableXform.prepare(pivotTable, {styles: model.styles});
      const xml = pivotTableXform.toXml(pivotTable);
      zip.append(xml, {name: `xl/pivotTables/pivotTable${pivotTable.id}.xml`});

      const relXml = relationshipsXform.toXml(pivotTable.rels);
      zip.append(relXml, {name: `xl/pivotTables/_rels/pivotTable${pivotTable.id}.xml.rels`});
    });
  }
  ...
  prepareModel(model, options) {
    ...
    model.worksheets.forEach(worksheet => {
      ...
      // assign unique ids to pivot tables
      worksheet.pivotTables.forEach(pivotTable => {
        pivotTableCount++;
        pivotTable.id = pivotTableCount;
        model.pivotTables.push(pivotTable);
      });

      // assign unique ids to pivot caches
      worksheet.pivotCaches.forEach(pivotCache => {
        pivotCacheCount++;
        pivotCache.id = pivotCacheCount;
        model.pivotCaches.push(pivotCache);
      });
